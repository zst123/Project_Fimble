/*
 *  https://github.com/kbiva/stm32f103_libs/tree/master/lib_MFRC522
 *  mfrc522.c
 *
...
/*********************** MODIFY FOR CYPRESS PSOC6 DEVICE *****************************************/
#define MFRC522_MOSI (P6_2) // DATA_OUT
#define MFRC522_MISO (P6_3) // DATA_IN
#define MFRC522_SCK (P9_6) // CLK
#define MFRC522_SDA (P9_7) // SS
#define MFRC522_RST (P13_7) // RESET

#define MFRC522_CS_LOW     cyhal_gpio_write(MFRC522_SDA, false);
#define MFRC522_CS_HIGH    cyhal_gpio_write(MFRC522_SDA, true);

#define MFRC522_CLK_LOW     cyhal_gpio_write(MFRC522_SCK, false);
#define MFRC522_CLK_HIGH    cyhal_gpio_write(MFRC522_SCK, true);

#define MFRC522_RST_LOW     cyhal_gpio_write(MFRC522_RST, false);
#define MFRC522_RST_HIGH    cyhal_gpio_write(MFRC522_RST, true);

void MFRC522_Init(void) {
	// Output pins
	cyhal_gpio_init(MFRC522_MOSI, CYHAL_GPIO_DIR_OUTPUT,
			CYHAL_GPIO_DRIVE_STRONG, false);
	cyhal_gpio_init(MFRC522_SCK, CYHAL_GPIO_DIR_OUTPUT,
			CYHAL_GPIO_DRIVE_STRONG, false);
	cyhal_gpio_init(MFRC522_SDA, CYHAL_GPIO_DIR_OUTPUT,
			CYHAL_GPIO_DRIVE_STRONG, false);
	cyhal_gpio_init(MFRC522_RST, CYHAL_GPIO_DIR_OUTPUT,
					CYHAL_GPIO_DRIVE_STRONG, false);

	// Input pin
	cyhal_gpio_init(MFRC522_MISO, CYHAL_GPIO_DIR_INPUT,
				CYHAL_GPIO_DRIVE_PULLUP, true);

	// Deselect device first
	MFRC522_CS_HIGH;
	PCD_Init();
}

// uint8_t debug_return = 0;
uint8_t spi_write(uint8_t _transmit) {
	// transmit the data via bitbang
	uint8_t _receive = 0;
	for(int i = 0; i < 8; i++) {
		cyhal_gpio_write(MFRC522_MOSI, (_transmit & 0x80) != 0); // Set MOSI
		MFRC522_CLK_HIGH; // SCK high
		_transmit <<= 1;
		_receive <<= 1;
		_receive |= cyhal_gpio_read(MFRC522_MISO);
		MFRC522_CLK_LOW; // SCK low
	}
...
void PCD_Init(void) {
  ...
  /* Reset MFRC522 */
  MFRC522_RST_LOW; // GPIO_ResetBits(GPIO_Reset,GPIO_Pin_Reset);
  vTaskDelay(10);
  MFRC522_RST_HIGH; //GPIO_SetBits(GPIO_Reset,GPIO_Pin_Reset);
...
void PCD_DumpVersionToSerial() {
	...
	char buf[50];
	snprintf(buf, 50, "Firmware Version: 0x%x \r\n", v);
	vLoggingPrintf(buf);

	// Lookup which version
	switch(v) {
		case 0x88: vLoggingPrintf(" = (clone) \r\n");  break;
		case 0x90: vLoggingPrintf(" = v0.0 \r\n");     break;
		case 0x91: vLoggingPrintf(" = v1.0 \r\n");     break;
		case 0x92: vLoggingPrintf(" = v2.0 \r\n");     break;
		case 0x12: vLoggingPrintf(" = counterfeit chip \r\n");     break;
		default:   vLoggingPrintf(" = (unknown) \r\n");
	}

	// When 0x00 or 0xFF is returned, communication probably failed
	if ((v == 0x00) || (v == 0xFF)) {
		vLoggingPrintf("WARNING: Communication failure, is the MFRC522 properly connected? \r\n");
	}
